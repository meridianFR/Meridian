 ...
 *
 * - `/app/journal*` : utilisateur connecté ET abonnement actif.
 * - `/app/*`        : utilisateur connecté (tableau de bord, accessible à tous).
...
export async function middleware(request: NextRequest) {
  ...
  // /app/journal* : abonnement actif obligatoire. Le reste (tableau de bord
  // /app, /abonnement) : être connecté suffit.
  if (pathname.startsWith("/app/journal")) {
    ...
    if (!isActive(sub)) {
      ...
      // alors passer pour que le layout journal réconcilie l'abonnement depuis Stripe.
